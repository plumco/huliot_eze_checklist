...
# Set premium wide-page theme layout matching Huliot styling
st.set_page_config(
...
def replace_text_in_paragraph(paragraph, old_text, new_text):
    """
    Safely replaces old_text with new_text inside a paragraph run-by-run.
    This preserves the original font formatting (bold, color, size, underlines).
    """
    if old_text in paragraph.text:
        # Check if the target is fully contained within a single run
        for run in paragraph.runs:
            if old_text in run.text:
                run.text = run.text.replace(old_text, new_text)
                return
        # Fallback if text is split across multiple runs
        paragraph.text = paragraph.text.replace(old_text, new_text)

def toggle_checkbox_in_paragraph(paragraph, target_phrase, check=True):
    """
    Finds a target phrase inside a paragraph and changes unicode checkbox characters
    from ☐ (unchecked) to ☑ (checked) or vice-versa.
    """
...
def process_document(template_bytes, data):
    """
    Loads the native Word document binary, processes paragraphs, lists, and tables 
    without changing formatting, and outputs the filled document stream.
    """
    ...
    def process_p(paragraph):
        ...
        # 2. Dynamic Summary Field Updates
        ...
        # 7. Building Configuration Options
        ...
        for c_type in ["PVC", "Cast Iron", "HDPE", "CPVC/UPVC/OTHER"]:
            if c_type in data['boq_compare']:
                toggle_checkbox_in_paragraph(paragraph, c_type, True)
        ...
        if "Toilet Sunken-" in paragraph.text and data['rte_toilet_sunken']:
            replace_text_in_paragraph(paragraph, "     ", f"{data['rte_toilet_mm']} mm")
        if "Kitchen Sunken-" in paragraph.text and data['rte_kitchen_sunken']:
            replace_text_in_paragraph(paragraph, "     ", f"{data['rte_kitchen_mm']} mm")
        if "Utility Sunken-" in paragraph.text and data['rte_utility_sunken']:
            replace_text_in_paragraph(paragraph, "     ", f"{data['rte_utility_mm']} mm")
    ...
    # Scan basic body paragraphs
    for p in doc.paragraphs:
        process_p(p)
    
    # Scan cells inside native tables to ensure replacements match
    for t in doc.tables:
        for r in t.rows:
            for c in r.cells:
                for p in c.paragraphs:
                    process_p(p)
                # Ensure nested tables are scanned
...
st.markdown("""
    <div style='background-color:#0f5132; padding:15px; border-radius:12px; margin-bottom:20px;'>
        <h1 style='color:white; margin:0; font-family:sans-serif;'>💼 Huliot India</h1>
        <p style='color:#d1e7dd; margin:5px 0 0 0; font-size:14px;'>Eze Checklist & Word Document Generator - Verbatim Layout Engine</p>
    </div>
""", unsafe_allow_value=True)

# Try auto-detecting original template in active workspace directory
fallback_filename = "Checklist -Questinorie for Huliot (Drainage and Water supply)_3.docx"
template_data = None

if os.path.exists(fallback_filename):
    with open(fallback_filename, "rb") as f:
        template_data = f.read()
    st.info(f"✔️ Automatically loaded original template format: `{fallback_filename}` from repository.")
else:
    st.warning("⚠️ Template file not found in repository root. Please upload it below to compile your data.")
...
if uploaded_template is not None:
    ...
    st.success("✔️ Custom template uploaded successfully!")
...
if template_data is not None:
    ...
    with tab2:
        ...
        with col1:
            ...
            drn_int_us = st.checkbox("Internal Toilets - Ultra Silent")
            drn_int_ht = st.checkbox("Internal Toilets - HT Pro")
            drn_vert_us = st.checkbox("Vertical/ External stack - Ultra Silent")
            drn_vert_ht = st.checkbox("Vertical/ External stack - HT Pro")
            drn_pod_us = st.checkbox("Podium/Diversion/ Basement – Ultra Silent")
            drn_pod_ht = st.checkbox("Podium/Diversion/ Basement - HT Pro")
            
        with col2:
            st.subheader("Water Supply Pipes & Fittings")
            ws_int_pipe_pert = st.checkbox("Internal Pipe - PERT/AL/PERT")
            ws_int_pipe_ppr = st.checkbox("Internal Pipe - PPR")
            ws_int_fit_ppsu = st.checkbox("Internal Fittings - PPSU")
            ws_int_fit_brass = st.checkbox("Internal Fittings - Brass")
            
            ws_ext_pipe_pert = st.checkbox("External Pipe - PERT/AL/PERT")
            ws_ext_pipe_ppr = st.checkbox("External Pipe - PPR")
            ws_ext_fit_ppsu = st.checkbox("External Fittings - PPSU")
            ws_ext_fit_brass = st.checkbox("External Fittings - Brass")
            
            ws_ter_pipe_pert = st.checkbox("Terrace Pipe - PERT/AL/PERT")
            ws_ter_pipe_ppr = st.checkbox("Terrace Pipe - PPR")
            ws_ter_fit_ppsu = st.checkbox("Terrace Fittings - PPSU")
            ws_ter_fit_brass = st.checkbox("Terrace Fittings - Brass")
    ...
    with tab3:
        ...
        with col2:
            ...
            boq_compare = st.multiselect("BOQ is compared with", ["PVC", "Cast Iron", "HDPE", "CPVC/UPVC/OTHER"])
            ...
            rte_ceiling = st.checkbox("Within false ceiling / suspended / underslung system")
            rte_toilet_sunken = st.checkbox("Toilet Sunken")
            rte_toilet_mm = st.text_input("Toilet Sunken depth in mm", "250") if rte_toilet_sunken else ""
            rte_kitchen_sunken = st.checkbox("Kitchen Sunken")
            rte_kitchen_mm = st.text_input("Kitchen Sunken depth in mm", "250") if rte_kitchen_sunken else ""
            rte_utility_sunken = st.checkbox("Utility Sunken")
            rte_utility_mm = st.text_input("Utility Sunken depth in mm", "250") if rte_utility_sunken else ""
    ...
    # Form mapping payload dictionary
    data = {
        ...
        'kitchen_sunken': f"Toilet: {rte_toilet_mm}mm, Kitchen: {rte_kitchen_mm}mm" if (rte_toilet_sunken or rte_kitchen_sunken) else "No",
    ...
    if st.button("🚀 Build Completed Word Document", type="primary"):
        with st.spinner("Processing templates..."):
            try:
                ...
                st.success("🎉 Successfully injected checklist variables into original Word layout!")
                st.download_button(
                ...
                st.error(f"Failed to process document. Please check template file compatibility. Error: {e}")